...
export const NavbarComponent = () => {
    ...
    return (
        <div className="flex flex-wrap items-center justify-between px-2 py-3 navbar-expand-lg top-0 absolute z-50 w-full ">
            <nav className={`${(open==="/dashboard"|| open.search("response")!==-1 || open.search("/interview")!==-1 || open==="/addinterview"  || open.search("share")!==-1 )?' hidden':'w-full'}`}>
                <div className=' min-w-full px-4 sm:px-6 lg:px-8 '>
                    <div className='flex items-center  pt-12 h-16'>
                        <div className='flex items-center justify-between w-full'>
                            <Link to="/" className='flex flex-row items-center my-2 lg:my-2 md:my-0 justify-center '>
                                <img src=""  className="w-24 h-24 " />
                                <div className='flex flex-col align-start md:hidden lg:flex'>
                                    <p
                                        className='flex-none text-gray-800 text-xl font-bold '
                                    >
                                        event
                                        <p className='text-gray-500  text-md font-normal '>Bot</p>
                                    </p>
                                    <p className='text-gray-600 text-xs'>get your events once</p>
                                </div>
                            </Link>
                                ...
                                {isAuth ?(
                                    <div className='flex ml-24'>
                                        {
                                            privateRoutes.map(Route=>(
                                                <div
                                                    key={Route.to}
                                                    className={`${open===Route.to?' border-gray-700 m-1':'m-1'}`}>
                                                    <Link
                                                        onClick={()=>{
                                                            setOpen(Route.to);
                                                        }}
                                                        ...
                                                        <div
                                                            className='mx-12 text-blue-700 font-semibold md:hidden lg:flex'
                                                        >
                                                            {Route.title}
                                                        </div>
                                                    </Link>
                                        ...
                                        <button className=" text-xl text-blue-700 font-semibold  " onClick={handleLogout}>Logout</button>
                                    </div>
                                ) : (
                                    <div className='ml-auto'>
                                        <Link
                                            ...
                                            className='flex items-center'>
                                            <div
                                                className='mx-12 text-gray-700 font-semibold md:hidden lg:flex'
                                            >Sign In
                                            </div>
                                        </Link>
                                    </div>
                ...
                <div className={`${toggle?'block md:hidden':'hidden '}`}>
                    {isAuth? (
                        ...
                        <div className='px-2 pt-2 pb-3 sm:px-3 ml-auto pl-auto '>
                            <Link
                                onClick={()=>{setOpen("/signin")}}
                                to="/signin"
                                className='flex items-center'>
